...
class drone(entities):
    ...
    def __init__(self,_canvas,_root, _drone_pos):
        self.canvas = _canvas
        self.root = _root
    ...
    def move_drone(self):
        ...
        if self.cur_y_drone<1:
            lower_limit_y = 0
        if self.cur_x_drone>g_var.dimension-2:
            upper_limit_x=0
        if self.cur_y_drone>g_var.dimension-2:
            upper_limit_y=0
        ...
        if self.drone_counter < g_var.movement_limit:
            self.root.after(g_var.turn_gap_time, self.move_drone)
